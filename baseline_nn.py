...
def main():
    ...
    # Run method
    train_net(data, args, TestProjNet, save_dir)
...
class TestProjNet(nn.Module):
    ...
    def forward(self, x, isTest=False):
        ...
        if isTest and self._if_project:
            A_eff, b_eff = self._data.get_Ab_effective(x)
            out = self.apply_projection(out[:,self._data.partial_unknown_vars], A_eff, b_eff)
